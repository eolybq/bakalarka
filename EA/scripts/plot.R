...
plot_data <- tibble(
    date = seq(as.Date("1999-01-01"), as.Date("2025-02-01"), "month"),
    sec_abs = window(ts_objects$sec_abs, start = c(1999, 1), end = c(2025,2)),
    ir = window(ts_objects$ir_ts, start = c(1999, 1), end = c(2025, 2))
)


plot_data$sec_abs <- plot_data$sec_abs / 1000000
...
ggplot() +
    ...
    scale_color_manual(
        labels = c(
        "ir" = "Sazba hlavních refinančních operací",
        "sec_abs" = "Držené dluhové tuzemské cenné papíry"
        ), 
        breaks = c("ir", "sec_abs"),
        values = c(
            "ir" = "steelblue",      
